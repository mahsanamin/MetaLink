...
sheet = book.sheet_by_name('Class_Type')
...
field_name_dict = {
    ID_IDX: 'id | (int)',
    CLASS_TYPE_IDX: 'class_type | ct (int)',
    HP_IDX: 'hp | (int)',
    ATK_IDX: 'atk | (int)',
    DEF_IDX: 'def | (int)',
    REC_IDX: 'rec | (int)',
    LESS_DMG_IDX: 'less_dmg | ldmg (int)',
    RESIST_IDX: 'resist | resi (int)',
}
# ---------------------------------------------
...
for row in xrange(start_row, sheet.nrows):
    ...
    for col in field_list:
        ...
        if col in (HP_IDX, ATK_IDX, DEF_IDX, REC_IDX, LESS_DMG_IDX):
            val = sheet.cell_value(row, col)
